...
const hiddenGuideline = "另外，必須避免使用陳腔濫調的情節（例如車禍、絕症、夢境結局），請構思更具原創性、更貼近真實人性的情節轉折。作品文學性高、過渡自然（不可以用時間詞過渡，例如「三年後」、「小時候」、「如今」，要多變）、意象豐富的敘事散文，情節不可零散，一件件事要慢慢渲染，延續寫下去，刻劃人物及情節都要求質不求量，才會動人。要重視人物之間的感情和交流，有不同的事件及回憶相互映襯突出主題，要善用通感及融情入景。必須避免使用身體部位作主語，例如「指尖」、「額角」。人物的對話不能有說教意味，要更有人情、人性。";
  
    let coreInstruction;
    if (structure === 'classic') {
        coreInstruction = `你是一位專業的作家，你的任務是要創作情感真摯、立意深刻、句式自然的DSE敘事抒情作品。必須用不同事件回憶去突顯映襯同一個主題，過渡自然，結構嚴謹，有明確的起承轉合。作品必須在${wordCount}字內。題目為「${topic}」。`;
    } else { // threeline
        coreInstruction = `你是一位專業的作家。你的任務是要創作情感真摯、立意深刻的作品、句式自然的DSE敘事抒情作品。必須用不同事件回憶去突顯映襯同一個主題，過渡自然，結構嚴謹。作品必須在${wordCount}字內，要用三線散敘寫作，題目為「${topic}」。`;
    }
